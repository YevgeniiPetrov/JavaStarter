...
/* Объясните, почему не рекомендуется использовать рекурсию для расчета факториала. Укажите слабые
места данного подхода. -
Рекурсивный расчет факторила влияет на память - каждый повторный вызов метода влияет на заполнение памяти и чем больше вызовов, тем больше памяти для этого выделяется.
Для решения подобной задачи, скорей всего, лучше использовать цикл с переменной, которая будет занимать меньше памяти, нежели рекрсивный вызов метода.
*/
